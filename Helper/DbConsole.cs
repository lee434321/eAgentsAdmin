...
namespace cn.magix.Helper
{
    ...
    public class DbTx
    {
        ...
        public DbVendor DbType { get { return this.vendor; } }
        #endregion
    ...
    public class DbConsole
    {
        ...
        public List<T> Retrieve<T>(Expression<Func<T, bool>> expr, string orderBy = "") where T : class
        {
            try
            {
                ...
                sql += " where " + (expr == null ? "1=1" : new ExprVistor().Parse(expr,m_tx.DbType));
                if (orderBy!="")                
                    sql += orderBy;
    ...
    public class ExprVistor : System.Linq.Expressions.ExpressionVisitor
    {
        ...
        private string Involve(Expression expr, bool eval = false)
        {
            ...
            switch (name)
            {
                ...
                case "NewExpression":
                    return InvolveNewExpression(expr as NewExpression);                    
                default:
                    break;
            }
            return "";
        ...
        private string InvolveNewExpression(NewExpression newExpression)
        {
        ...
        /// <param name="eval">是否取值变量值，true：取值；false：不取。默认false</param>
        /// <returns></returns>
    ...
    ///     predicate = PredicateBuilder.True<Employee>();此语句表示若直接解析将返回 1=1，用来接在where之后，完成解析
    /// 2.使用And或Or拼接其他条件,如：
    ///     predicate = predicate.And(x=>x.Name=="derek");此语句表示在原来的基础上，拼接一个And子句，将返回
